...
vm1 = glmer(infls ~ treatment + stress + treatment:stress + (stress|block), offset = final_count, data = v, family = poisson)
vm1
...
vresults$upper_se = vresults$lsmean + vresults$SE
vresults$lower_se = vresults$lsmean - vresults$SE 
vresults$bt_mean = exp(vresults$lsmean)
vresults$bt_upper_se = exp(vresults$upper_se)
vresults$bt_lower_se = exp(vresults$lower_se)
vresults
...
vm2 = glm( infls ~ stress + treatment + stress:treatment, offset = final_count, data = v, family = 'poisson')
df1 = expand.grid(stress = c('high', 'low'), treatment = c('BC', 'MC', 'MR'), final_count = 1)
...
blsmeans
bresults = blsmeans$'treatment:stress lsmeans'
bresults

bresults$upper_se = bresults$lsmean + bresults$SE
bresults$lower_se = bresults$lsmean - bresults$SE 
bresults$bt_mean = inv.logit(bresults$lsmean)
bresults$bt_upper_se = inv.logit(bresults$upper_se)
bresults$bt_lower_se = inv.logit(bresults$lower_se)
bresults

brom_plot = ggplot(data = bresults, aes(x = stress, y = bt_mean, ymin = bt_lower_se, ymax = bt_upper_se, 
                                   group = treatment, color = treatment))
...
bm2 = glm(cbind(final_count, trials-final_count) ~ treatment + stress + treatment:stress, data = b, family = 'binomial')
df2 = expand.grid(stress = c('high', 'low'), treatment = c('BC', 'MC', 'MR'))
df2 = df2[order(df2$stress), ]

bt_predicted = data.frame(predict(bm2,df2, type = 'response', se.fit= TRUE ))
df2 = cbind(df2, bt_predicted)
df2$fit
bresults$bt_mean

brom_plot2 = ggplot(data = df2, aes(x = stress, y = fit, ymin = df2$fit - df2$se, ymax = df2$fit + df2$se, 
                                        group = treatment, color = treatment))
brom_plot2 + geom_point(position = position_dodge(width = 0.2)) + 
  geom_line(stat = 'identity', position = position_dodge( width = 0.2)) + 
  geom_errorbar ( position = position_dodge(), width = 0.2) +  ylim(0,1) + 
  theme_bw() 
