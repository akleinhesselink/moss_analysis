...
### Table 2. Multiple comparison test Bromus survival. Results are given on the log odds ratio (not the response) scale. P-values are adjusted by Sidak method for three tests. 
```{r echo=FALSE, warning=FALSE, message = FALSE}

bm_emeans <- emmeans(bm1, pairwise ~ treatment | position, adj = 'sidak' )

make_mct_table(bm_emeans) %>%
  pander()
...
make_mct_table(bf_emeans) %>% pander()
...
make_mct_table(vm_emeans_no_interaction) %>% pander()
